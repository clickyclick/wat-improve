...
class Review extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.get("http://localhost:3001/api/review/" + this.state.id).then((res) => {
            ...
            if (res.data.data.reviewAreas && res.data.data.name) {
                ...
                this.setState({ name: res.data.data.name, message: res.data.data.message, reviewAreas: res.data.data.reviewAreas, completeReview: completeReview })
            }
        });
    }
    ...
    completeReview = () => {
        ...
        axios.post("http://localhost:3001/api/submission", {
            id: id,
    ...
    handleChange = name => event => {
        let newCompletedReview = this.state.completeReview;
        newCompletedReview[name] = event.target.value
        this.setState({ completeReview: newCompletedReview });
    };
