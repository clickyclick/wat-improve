...
class Result extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.get(config.envConfig.serverURL + "/api/submission/" + this.state.id).then((res) => {
            console.log(res.data.data.completeReview);
            this.setState({reviewAreas: res.data.data.reviewAreas, completeReview: res.data.data.completeReview })
        });
    }

    render() {
        console.log(this.state.completeReview);
        console.log(this.state.completeReview.quality === '3');
